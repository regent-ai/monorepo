...
- `express` - Node-style HTTP server built on Express with `@regent/agent-kit-express`
...
```bash
bunx @regent/cli <app-name> [options]
...
bunx @regent/cli@latest my-agent --template=identity

# With Solana payment network
bunx @regent/cli@latest my-agent --network=solana-devnet

# With Base mainnet
bunx @regent/cli@latest my-agent --network=base

# Identity template with Solana payments
bunx @regent/cli@latest my-agent --template=identity --network=solana

# With Express adapter
bunx @regent/cli@latest my-agent --adapter=express --template=blank

# With Hono adapter
bunx @regent/cli@latest my-agent --adapter=hono --template=blank

# With TanStack UI (full dashboard)
bunx @regent/cli@latest my-agent --adapter=tanstack-ui --template=blank

# With TanStack headless (API only, no UI)
bunx @regent/cli@latest my-agent --adapter=tanstack-headless --template=blank

# Auto-install dependencies
bunx @regent/cli@latest my-agent --install

# Non-interactive with defaults
bunx @regent/cli@latest my-agent --template=blank --wizard=no
```
...
```bash
# Solana devnet
bunx @regent/cli my-agent --network=solana-devnet --non-interactive

# Base mainnet
bunx @regent/cli my-agent --network=base --non-interactive
```
...
bunx @regent/cli@latest verified-agent \
  --template=identity \
  --non-interactive \
  --install \
...
# AxLLM template
bunx @regent/cli@latest ai-agent \
  --template=axllm \
  --non-interactive \
...
- [`@regent/core`](../core/README.md) - Core agent runtime
- [`@regent/erc8004`](../identity/README.md) - ERC-8004 identity
