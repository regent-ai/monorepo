...
const REGENT_BANNER = [
  ...
  '',
  'Regent agent scaffolding toolkit',
];
...
function printHelp(logger: RunLogger) {
  logger.log('Usage: bunx @regent/cli <app-name> [options]');
  logger.log('');
  logger.log('Options:');
  logger.log(
  ...
  logger.log('');
  logger.log('Examples:');
  logger.log('  bunx @regent/cli my-agent');
  logger.log('  bunx @regent/cli my-agent --network=solana-devnet');
  logger.log('  bunx @regent/cli my-agent --template=identity --install');
  logger.log('  bunx @regent/cli my-agent --wizard=no');
  logger.log('');
  logger.log('Non-interactive with template arguments:');
  logger.log('  bunx @regent/cli my-agent --template=identity \\');
...
function printBanner(logger: RunLogger) {
  REGENT_BANNER.forEach(line => logger.log(line));
}
