...
`@regent/core` is the core runtime for building AI agents with typed entrypoints, discovery endpoints, monetization hooks, and trust metadata. It provides the shared runtime logic used by adapter packages like `@regent/hono` and `@regent/tanstack`.

**Note:** For most use cases, you'll want to use one of the adapter packages (`@regent/hono` or `@regent/tanstack`) rather than importing from this core package directly.
...
**Note:** HTTP-specific functionality (handlers, invoke, stream) is provided by the `@regent/http` extension, not the core package.
...
import type { EntrypointDef, AgentMeta } from '@regent/core';
```
...
- `@regent/core` — main exports including types (EntrypointDef, AgentMeta, etc.)
- `@regent/core/utils` — focused helpers
- `@regent/core/axllm` — AxLLM client integration

## Core Concepts

### Core Runtime

This package provides the core runtime logic via an extension-based API. Use `createAgent()` to build a runtime with extensions, then pass it to adapter packages like `@regent/hono` and `@regent/tanstack`.
...
Regent Agents supports payment receiving on multiple blockchain networks:
...
const { app, addEntrypoint } = createAgentApp(
  ...
  },
  {
    payments: {
      ...
      facilitatorUrl: 'https://facilitator.regent.cx',
    },
  }
);
...
See [`@regent/payments` documentation](../payments/README.md) for complete examples, storage options, and API reference.
...
`resolvePrice(entrypoint, payments, kind)` (from `@regent/payments`) returns the price or `null`.

For authenticated wallet access, pair your agent with
`@regent/agent-auth` and reuse the generated SDK surface:

```ts
import { AgentRuntime } from '@regent/agent-auth';
import { createRuntimePaymentContext } from '@regent/payments';
...
const { runtime } = await AgentRuntime.load({
  ...
  loader: {
    overrides: {
      baseUrl: process.env.REGENT_API_URL,
...
Trust metadata is modelled by `TrustConfig`. For ERC-8004 identity management, use the dedicated `@regent/identity` package:

```ts
import { createAgentIdentity, getTrustConfig } from '@regent/identity';
...
See [`@regent/identity` documentation](../@regent/identity/README.md) for complete examples and API reference.
...
```ts
import { fetchAndInvoke } from '@regent/a2a';
