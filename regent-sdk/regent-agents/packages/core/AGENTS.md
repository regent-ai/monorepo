...
import type { EntrypointDef, AgentMeta } from '@regent/types/core';
import type { AP2Config } from '@regent/types/ap2';
```

Subpath exports are available:

- `@regent/core` — core runtime API
- `@regent/core/utils` — shared helpers (e.g., JSON schema conversion)
- `@regent/http` — HTTP extension (SSE, invoke/stream helpers)
- Adapter packages: `@regent/hono`, `@regent/express`, `@regent/tanstack`
...
import { identity } from '@regent/identity';
import { createAgentApp } from '@regent/hono';
...
Use `@regent/identity` for ERC-8004 identity. The recommended path is `createAgentIdentity` (auto-register + trust config); advanced workflows can use `createIdentityRegistryClient`.

```ts
import {
  createAgentIdentity,
} from '@regent/identity';
