...
async function setupAgentRuntime(options: {
  ...
  const baseUrl =
    process.env.REGENT_AGENT_BASE_URL ??
    process.env.AGENT_AUTH_BASE_URL ??
    process.env.API_BASE_URL;
  const agentRef =
    process.env.REGENT_AGENT_AGENT_REF ??
    process.env.AGENT_REF;
  const credentialId =
    process.env.REGENT_AGENT_CREDENTIAL_ID ?? process.env.CREDENTIAL_ID;
  const refreshToken =
    process.env.REGENT_AGENT_REFRESH_TOKEN ?? process.env.REFRESH_TOKEN;
  const scopes =
    parseScopes(process.env.REGENT_AGENT_SCOPES) ??
  ...
  if (!baseUrl || !agentRef || !credentialId) {
    console.warn(
      '[examples] AgentRuntime auth skipped; provide REGENT_AGENT_BASE_URL, REGENT_AGENT_AGENT_REF, and REGENT_AGENT_CREDENTIAL_ID to enable challenge flow'
    );
    return null;
  }
    ...
    const { app, addEntrypoint } = createAgentApp(
      {
        ...
        version: '0.1.0',
        description:
          'Demonstrates every feature exposed by Regent Agents. A2A, x402, ERC-8004, and more.',
      },
      {
