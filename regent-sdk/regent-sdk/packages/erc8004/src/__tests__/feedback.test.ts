 ...
 *
 * REQUIRES: AGENT_PRIVATE_KEY, CLIENT_PRIVATE_KEY, and PINATA_JWT environment variables
 * Skip these tests in CI or when credentials are not available.
 */

import { describe, it, expect, beforeAll } from 'bun:test';
import { SDK } from '../index.js';
import { CHAIN_ID, RPC_URL, AGENT_PRIVATE_KEY, PINATA_JWT, AGENT_ID, CLIENT_PRIVATE_KEY, printConfig } from './config.js';

// Skip this entire test suite if required credentials are missing
const SKIP_REASON = !CLIENT_PRIVATE_KEY
  ? 'CLIENT_PRIVATE_KEY not set'
  : !AGENT_PRIVATE_KEY
...
if (SKIP_REASON) {
  console.log(`⚠️  Skipping feedback tests: ${SKIP_REASON}`);
}

describeWithCredentials('Agent Feedback Flow with IPFS Pin', () => {
  let agentSdk: SDK;
  let clientSdk: SDK;
