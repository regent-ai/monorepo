 ...
 *
 * REQUIRES: AGENT_PRIVATE_KEY and PINATA_JWT environment variables
 * Skip these tests in CI or when credentials are not available.
 */

import { describe, it, expect, beforeAll } from 'bun:test';
import { SDK } from '../index.js';
import { CHAIN_ID, RPC_URL, AGENT_PRIVATE_KEY, PINATA_JWT, printConfig } from './config.js';

// Skip this entire test suite if required credentials are missing
const SKIP_REASON = !AGENT_PRIVATE_KEY
...
if (SKIP_REASON) {
  console.log(`⚠️  Skipping IPFS registration tests: ${SKIP_REASON}`);
}

describeWithCredentials('Agent Registration with IPFS Pin', () => {
  let sdk: SDK;
