 ...
 *
 * REQUIRES: AGENT_PRIVATE_KEY environment variable (writes to blockchain)
 * Skip these tests in CI or when credentials are not available.
 */
...
import { CHAIN_ID, RPC_URL, AGENT_PRIVATE_KEY, printConfig } from './config.js';

// Skip this entire test suite if required credentials are missing
const SKIP_REASON = !AGENT_PRIVATE_KEY ? 'AGENT_PRIVATE_KEY not set' : null;
const describeWithCredentials = SKIP_REASON ? describe.skip : describe;
...
if (SKIP_REASON) {
  console.log(`⚠️  Skipping HTTP registration tests: ${SKIP_REASON}`);
}

describeWithCredentials('Agent Registration with HTTP URI', () => {
  let sdk: SDK;
  ...
  it('should update agent and re-register', async () => {
    ...
    const filepathUpdated = path.join(import.meta.dir, filenameUpdated);
