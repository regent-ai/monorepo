/**
 * Feedback management system for @regent/erc8004
 */

import { keccak256, toHex, toBytes, hexToBytes, bytesToHex, fromHex, hexToString } from 'viem';
import type {
  Feedback,
...
import type { Web3Client, ContractInstance } from './web3-client.js';
...
/**
 * Manages feedback operations for @regent/erc8004
 */
export class FeedbackManager {
  ...
  constructor(
    ...
    private reputationRegistry?: ContractInstance,
    private identityRegistry?: ContractInstance,
   ...
   */
  setReputationRegistry(registry: ContractInstance): void {
   ...
   */
  setIdentityRegistry(registry: ContractInstance): void {
  ...
  async signFeedbackAuth(
    ...
    // Sign the hash (viem will add the Ethereum signed message prefix automatically)
    const signature = await this.web3Client.signMessage(hexToBytes(messageHash));
  ...
  async giveFeedback(
    ...
    try {
      const txHash = await this.web3Client.transactContract(
        ...
        feedbackUri,
        feedbackHash,
        hexToBytes(authBytes.startsWith('0x') ? authBytes as `0x${string}` : `0x${authBytes}`)
      );
  ...
  private async _getFeedbackFromBlockchain(
    ...
    try {
      const result = await this.web3Client.callContract(
        this.reputationRegistry,
        'readFeedback',
        BigInt(tokenId),
        clientAddress,
      ...
      const [score, tag1Bytes, tag2Bytes, isRevoked] = result;

      const tags = this._bytes32ToTags(tag1Bytes as string, tag2Bytes as string);
  ...
  private _hexBytes32ToTags(tag1: string, tag2: string): string[] {
    ...
    if (tag1 && tag1 !== '0x' + '00'.repeat(32)) {
      ...
      if (!tag1.startsWith('0x')) {
        ...
        try {
          const hexBytes = hexToBytes(tag1 as `0x${string}`);
          const filtered = hexBytes.filter((b) => b !== 0);
          const tag1Str = new TextDecoder('utf-8', { fatal: false }).decode(filtered);
          if (tag1Str) {
            tags.push(tag1Str);
          }
        } catch {
    ...
    if (tag2 && tag2 !== '0x' + '00'.repeat(32)) {
      ...
      if (!tag2.startsWith('0x')) {
        ...
        // Try to convert from hex bytes32 (on-chain format)
        try {
          const hexBytes = hexToBytes(tag2 as `0x${string}`);
          const filtered = hexBytes.filter((b) => b !== 0);
          const tag2Str = new TextDecoder('utf-8', { fatal: false }).decode(filtered);
          if (tag2Str) {
            tags.push(tag2Str);
          }
        } catch {
   ...
   */
  private _stringToBytes32(text: string): `0x${string}` {
    if (!text) {
  ...
  private _bytes32ToTags(tag1Bytes: string, tag2Bytes: string): string[] {
    ...
    if (tag1Bytes && tag1Bytes !== '0x' + '00'.repeat(32)) {
      try {
        const bytes = hexToBytes(tag1Bytes as `0x${string}`);
        const filtered = bytes.filter((b) => b !== 0);
        const tag1 = new TextDecoder('utf-8', { fatal: false }).decode(filtered).trim();
        if (tag1) {
          tags.push(tag1);
        }
      } catch {
    ...
    if (tag2Bytes && tag2Bytes !== '0x' + '00'.repeat(32)) {
      try {
        const bytes = hexToBytes(tag2Bytes as `0x${string}`);
        const filtered = bytes.filter((b) => b !== 0);
        const tag2 = new TextDecoder('utf-8', { fatal: false }).decode(filtered).trim();
        if (tag2) {
          tags.push(tag2);
        }
      } catch {
  ...
  async getReputationSummary(
    ...
    try {
      ...
      const summaryResult = await this.web3Client.callContract(
        this.reputationRegistry,
        'getSummary',
        BigInt(tokenId),
        clients,
      ...
      const [count, averageScore] = summaryResult;
