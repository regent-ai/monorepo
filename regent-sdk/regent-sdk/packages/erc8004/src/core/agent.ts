...
import { toHex, type TransactionReceipt, decodeEventLog, type Log } from 'viem';
import type {
  RegistrationFile,
  Endpoint,
...
export class Agent {
  ...
  async registerIPFS(): Promise<RegistrationFile> {
    ...
    if (this.registrationFile.agentId) {
      ...
      const identityRegistryAddress = this.sdk.getIdentityRegistry().address;
  ...
  private _extractAgentIdFromReceipt(receipt: TransactionReceipt): bigint {
    ...
    for (const log of receipt.logs || []) {
      try {
        // Try parsing as Registered event using viem's decodeEventLog
        const decoded = decodeEventLog({
          abi: identityRegistry.abi,
          topics: log.topics,
          data: log.data,
        });
        if (decoded && decoded.eventName === 'Registered') {
        ...
        try {
          ...
          if (topics.length >= 4) {
            ...
            if (topic0 && topic3 && (topic0 === transferEventTopic || topic0.toLowerCase() === transferEventTopic.toLowerCase())) {
