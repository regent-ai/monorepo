/**
 * Main SDK class for @regent/erc8004
 */
...
import { Web3Client, type TransactionOptions, type ContractInstance } from './web3-client.js';
...
export interface SDKConfig {
  chainId: ChainId;
  rpcUrl: string;
  signer?: string; // Private key string (optional for read-only operations)
...
export class SDK {
  ...
  private _identityRegistry?: ContractInstance;
  private _reputationRegistry?: ContractInstance;
  private _validationRegistry?: ContractInstance;
   ...
   */
  getIdentityRegistry(): ContractInstance {
   ...
   */
  getReputationRegistry(): ContractInstance {
   ...
   */
  getValidationRegistry(): ContractInstance {
  ...
  async loadAgent(agentId: AgentId): Promise<Agent> {
    ...
    try {
      ...
      tokenUri = await this._web3Client.callContract(identityRegistry, 'tokenURI', BigInt(tokenId)) as string;
    } catch (error) {
  ...
  async isAgentOwner(agentId: AgentId, address: Address): Promise<boolean> {
    ...
    const identityRegistry = this.getIdentityRegistry();
    const owner = await this._web3Client.callContract(identityRegistry, 'ownerOf', BigInt(tokenId)) as string;
  ...
  async getAgentOwner(agentId: AgentId): Promise<Address> {
    const { tokenId } = parseAgentId(agentId);
    const identityRegistry = this.getIdentityRegistry();
    return await this._web3Client.callContract(identityRegistry, 'ownerOf', BigInt(tokenId)) as Address;
  }
