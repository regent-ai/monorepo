...
Shared TypeScript type definitions for the Regent agent framework.

## Overview

This package provides the core type definitions used across all `@regent/*` packages. It has **zero dependencies** on other `@regent` packages, making it a pure leaf package in the dependency graph.
...
This package has no dependencies on other `@regent` packages. It only depends on:
