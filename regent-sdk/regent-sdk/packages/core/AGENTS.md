...
Core agent runtime + helpers for building typed agent apps:
...
This package is part of the Regent SDK monorepo. Typical imports:

```ts
import { createAgent } from '@regent/core';
import { http } from '@regent/http';
import { createAgentApp } from '@regent/hono';
import { paymentsFromEnv } from '@regent/x402';
import type { AgentMeta, EntrypointDef } from '@regent/types';
```

See also:

- `@regent/x402` — payments helpers (`paymentsFromEnv`, runtime payment context)
- `@regent/wallet` — wallets helpers (`walletsFromEnv`)
- `@regent/erc8004` — ERC-8004 identity + discovery + reputation SDK
...
import { identity } from '@regent/erc8004';
import { createAgentApp } from '@regent/hono';
...
> `DEFAULT_PRICE`, `REGENT_API_URL`, etc.) are still respected. Values supplied
...
`@regent/agent-auth` and reuse its `AgentRuntime` helpers instead of the
...
### ERC-8004 Identity Helpers (SDK)

Use `@regent/erc8004` for registry/subgraph/IPFS helpers and `TrustConfig` from `@regent/types/identity` to surface identity metadata in your agent card/manifest.

```ts
import { SDK as RegentErc8004SDK } from '@regent/erc8004';
import type { TrustConfig } from '@regent/types/identity';
```
...
- `CreateAgentAppReturn<TApp>`: `{ app: TApp, runtime, agent, addEntrypoint, config }` (generic type in `@regent/types/core`)
