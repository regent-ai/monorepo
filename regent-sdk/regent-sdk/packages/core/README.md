...
Regent supports payment receiving on multiple blockchain networks:
...
See [`@regent/x402` documentation](../payments/README.md) for complete examples, storage options, and API reference.
...
`resolvePrice(entrypoint, payments, kind)` (from `@regent/x402`) returns the price or `null`.
...
import { createRuntimePaymentContext } from '@regent/x402';
...
Trust metadata is modelled by `TrustConfig`. For ERC-8004 identity management, use the dedicated `@regent/erc8004` package:

```ts
import { createAgentIdentity, getTrustConfig } from '@regent/erc8004';
...
See [`@regent/erc8004` documentation](../@regent/erc8004/README.md) for complete examples and API reference.
