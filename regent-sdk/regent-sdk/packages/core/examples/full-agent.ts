...
async function setupAgentRuntime(options: {
  ...
  const credentialId =
    process.env.REGENT_AGENT_CREDENTIAL_ID ??
    process.env.CREDENTIAL_ID;
  const refreshToken =
    process.env.REGENT_AGENT_REFRESH_TOKEN ??
    ...
    const { app, addEntrypoint } = createAgentApp(
      {
        ...
        version: '0.1.0',
        description:
          'Demonstrates every feature exposed by @regent/core. A2A, x402, ERC-8004, and more.',
      },
      {
